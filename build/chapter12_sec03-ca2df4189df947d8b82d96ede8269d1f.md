...
Klassifikationsaufgabe und lassen sie visualisieren.
...
`MLPClassifier`. Wir trennen die Daten wie üblich in Trainings- und Testdaten,
lassen das neuronale Netz tranieren und geben die Scores für die Trainings- und
Testdaten aus.
...
Der Score für die Trainingsdaten 0.91 ist sehr gut, aber der Score für die
Testdaten ist mit 0.84 weniger gut im Vergleich zu dem Tesscore. Vermutlich
liegt Overfitting vor. Neuronale Netze können zwar komplexe Strukturen in den
Daten abbilden, brauchen aber auch viele Datenpunkte, damit beim Training kein
Overfitting auftritt. Mit wie vielen Neuronen (und damit Gewichten) arbeitet das
aktuelle neuronale Netz eigentlich?
...
ein Tupel mit ganzen Zahlen übergeben. Dabei ist die erste Zahl in dem Tupel die
Anzahl der Neuronen in der ersten versteckten Schicht an, die zweite Zahl ist
die Anzahl der Neuronen in der zweiten Schicht und immer so weiter. In der
Standardeinstellung wird also eine versteckte Schicht mit 100 Neuronen
verwendet.

Bei zwei Merkmalen, einer versteckten Schicht mit 100 Neuronen und einer Ausgabe
...
nicht, um Overfitting zu vermeiden. Wir reduzieren daher die Anzahl der Neuronen
in der versteckten Zwischenschicht auf 10.

```{code-cell} ipython3
# Auswahl des Modells
neuronales_netz = MLPClassifier(hidden_layer_sizes=(10,))
