...
rechts eingerückt. Die Multiplikation der Inputs $x_i$ mit den Gewichten $w_i$
wird durch Kanten dargestellt. Die einzelnen Summanden $x_i w_i$ treffen sich
sozusagen im mittleren Kreis, wo die gewichtete Summe $\sum_{j=0}^{n} x_j w_j$
gebildet wird. Auf die gewichtete Summe wird dann eine Aktivierungsfunktion
angewendet, was durch einen weiteren Kreis dargestellt wird. Um zu
verdeutlichen, dass hier zwei mathematische Operationen durchgeführt werden,
sind die Kreise hellgrau eingefärbt. Das Ergebnis, der Output $\hat{y}$ wird
dann berechnet und wiederum als (dunkelblauer) Kreis gezeichnet.
