...
Aktivierungsfunktion wurde verwendet? Die sentralen Bestandteile eines
...
x 100 + 1 + 1 = 202 Gewichte. Gleichzeitig haben wir nur 100 Datenpunkte, also
nur halb so viele Informationen wie Gewichte. Damit ist die Gefahr groß, dass
Overfitting vorliegt. Wir wählen zwei versteckte Schichten mit jeweils fünf
Neuronen.
...
zu klassifizieren. Probieren wir zwei versteckte Schichten mit jeweils 10
Neuronen aus.
...
Die Scores für Trainings- und Testdaten sind sehr gut und die
Entscheidungsgrenzen sind plausibel.
...
eine große Herausforderung. Zum Abschluss verwenden wir noch die Gittersuche mit
Kreuzvalidierung, um die Architektur des neuronalen Netzes zu optimieren. Neben
der Anzahl der versteckten Schichten ändern wir auch noch die
Aktivierungsfunktion.

Ein komplettes Beispiel für das Training eines neuronalen Netzes lautet wie folgt:
...
base_model = MLPClassifier(solver='lbfgs', random_state=42)
...
print("\n=== Ergebnisse der Gittersuche ===")
print(f"Beste Parameter: {grid_search.best_params_}")
print(f"Bester Score (Kreuzvalidierung): {grid_search.best_score_:.3f}")
...
# Alle Ergebnisse als DataFrame anzeigen
results_df = pd.DataFrame(grid_search.cv_results_)
results_df = results_df.sort_values('rank_test_score')

# Wichtigste Spalten auswählen und formatieren
display_df = results_df[['param_hidden_layer_sizes', 'param_alpha', 
                          'mean_test_score', 'std_test_score', 'rank_test_score']].head(10)
display_df.columns = ['Architektur', 'Alpha', 'Mittlerer Score', 
                      'Standardabweichung', 'Rang']
...
results_plot['architecture'] = results_plot['param_hidden_layer_sizes'].astype(str)

fig = px.box(results_plot, 
             x='architecture', 
             y='mean_test_score',
             title='Vergleich verschiedener Netzarchitekturen',
             labels={'architecture': 'Architektur (hidden_layer_sizes)',
                     'mean_test_score': 'Mittlerer Score (Kreuzvalidierung)'},
             points='all')
fig.update_layout(xaxis_tickangle=-45)
fig.show()

# Visualisierung: Einfluss von Alpha
fig2 = px.scatter(results_plot, 
                  x='param_alpha', 
                  y='mean_test_score',
                  color='architecture',
                  title='Einfluss des Regularisierungsparameters Alpha',
                  labels={'param_alpha': 'Alpha (Regularisierung)',
                          'mean_test_score': 'Mittlerer Score'})
fig2.update_xaxis(type='log')  # Logarithmische Skala für Alpha
fig2.show()
```
